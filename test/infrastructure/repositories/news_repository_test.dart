  ...
  group('getTopHeadlines', () {
    ...
    test(
      ...
      () async {
        // arrange
        when(
          () => mockRemoteDataSource.getTopHeadlines(country: any(named: 'country')),
        ).thenAnswer((_) async => tNewsResponseModel);

        // act
        final result = await repository.getTopHeadlines(country: 'au');

        // assert
        expect(result, Right(tNewsResponse));
        verify(() => mockRemoteDataSource.getTopHeadlines(country: any(named: 'country')));
        verifyNoMoreInteractions(mockRemoteDataSource);
      },
    );

    test('should return NewsResponse with custom parameters', () async {
      // arrange
      when(
        () => mockRemoteDataSource.getTopHeadlines(country: 'us', pageSize: 5),
      ).thenAnswer((_) async => tNewsResponseModel);
    ...
    test('should return Left when API returns non-ok status', () async {
      // arrange
    ...
    test('should return Left when no articles are found', () async {
      // arrange
      ...
      expect(
        result,
        const Left('No news articles found for the specified criteria'),
      );
      verify(() => mockRemoteDataSource.getTopHeadlines(country: any(named: 'country')));
    });
    ...
    test(
      ...
      () async {
        ...
        expect(
          result,
          const Left('API rate limit exceeded. Please try again later.'),
        );
        verify(() => mockRemoteDataSource.getTopHeadlines(country: any(named: 'country')));
      },
    );
    ...
    test(
      ...
      () async {
        ...
        expect(
          result,
          const Left('Unexpected error: Exception: Some unexpected error'),
        );
        verify(() => mockRemoteDataSource.getTopHeadlines(country: any(named: 'country')));
      },
    );
